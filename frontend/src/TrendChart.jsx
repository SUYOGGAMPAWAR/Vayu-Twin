  ...
  if (!cityName) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-500 text-sm">
        ...
        <p className="text-xs text-slate-600 mt-1">Click any city from the list or map to view 24-hour trends.</p>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <div className="h-full flex items-center justify-center text-slate-500 text-sm">
        Loading trend data for {cityName}...
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-xs text-slate-400">Comparing Sensors vs Satellite HCHO</span>
      </div>
